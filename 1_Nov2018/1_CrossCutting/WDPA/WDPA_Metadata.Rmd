...
e. The global extent of protected area was calculated in M ha as the total coverage in each subregion for terrestrial and marine sites. These data were also used to calculate protected area coverage separately for terrestrial and marine. Mollweide projection was used for area calculations. Results are saved as 'WDPA_timeseries.csv'.  

**Calculate ICCA coverage**  
R script: WDPA_calculate_ICCA.R  

a.  Using the monthly release from August 2018,data were filtered to sites designated as ICCAs by selecting sites in which the 'GOV_TYPE' was either "Local communities" or "Indigenous peoples".  
b.  For point data, sites with no reported area were excluded. For remaining sites, reported area was used to calculate the radius of a circle with the given area and used to buffer spatial points into polygons. 
c.  Buffered points and polygons were combined and then spatially aggregated by year and ISO3 code. Due to multiple designations and misreporting of areas, protected areas can frequently overlap in the WDPA. This spatial union accounts for this and prevents double-counting of areas under the same ISO3.  
d.  Total ICCA coverage is calculated in M ha under a Mollweide projection and as a percent of estimated global extent of ICCA (400-800 M ha). Results are saved as 'ICCA_timeseries.csv'.  
