...
Dim_Context_State_Wildlife_A <- 
  data.frame(Indicator_Type_Key="GCS_WL_A",
             ...
             Indicator_Label="Living Planet Index",
...
Fact_Context_State_Wildlife_A <-
  ...
  transmute(Year_Key=Year,
            ...
            Indicator_Type_Key=rep(Dim_Context_State_Wildlife_A$Indicator_Type_Key,length(Year)),
...
Dim_Context_State_Wildlife_B <- 
  data.frame(Indicator_Type_Key="GCS_WL_B",
             ...
             Indicator_Label="Red List Index",
             Panel_Label="Global Biodiversity",
             Panel="State",
...
Fact_Context_State_Wildlife_B <-
  ...
  transmute(Year_Key=Year,
            Practice_Key=rep(practice_key_ref$id[practice_key_ref$practice_name=="Wildlife"],length(Year)),
            Indicator_Type_Key=rep(Dim_Context_State_Wildlife_B$Indicator_Type_Key,length(Year)),
            Indicator_Value=Value,
            Indicator_Upper_Value=Upper,
            Indicator_Lower_Value=Lower)
...
Fact_Context_Threat_Wildlife_A <-
  ...
  transmute(Year_Key=Year,
            Practice_Key=rep(practice_key_ref$id[practice_key_ref$practice_name=="Wildlife"],length(Year)),
            Indicator_Type_Key=rep(Dim_Context_Threat_Wildlife_A$Indicator_Type_Key,length(Year)),
            Indicator_Value=Value,
...
Dim_Context_Response_Wildlife_B <- 
  data.frame(Indicator_Type_Key="GCR_WL_B",
             ...
             Indicator_Label="Community Conserved",
             Panel_Label="Habitat Protection",
             Panel="Response",
             Indicator_Subcategory="B",
             Indicator_Unit="M ha",
...
Fact_Global_2030_Outcome1_Wildlife_A <-
  data.frame(Year_Key=NA,
            Practice_Key=rep(practice_key_ref$id[practice_key_ref$practice_name=="Wildlife"],length(1)),
            Indicator_Type_Key=rep(Dim_Global_2030_Outcome1_Wildlife_A$Indicator_Type_Key, length(1)),
...
Dim_Global_2030_Outcome1_Wildlife_B <- 
  data.frame(Indicator_Type_Key="OUT1_WL_B",
             Indicator_Name="Percent of total protected area that has a management plan (M ha PAs with mgmt plans / M ha PAs)",
             Indicator_Label="Actively Managed",
             Indicator_Subcategory="B",
...
Dim_Global_2030_Outcome1_Wildlife_C <- 
  data.frame(Indicator_Type_Key="OUT1_WL_C",
             ...
             Indicator_Label="Biologically Important",
...
Fact_Initiative_Indicator_Wildlife <-
  left_join(dim.initiative.indicators.wildlife,dim.initiatives.wildlife,by="Initiative") %>%
